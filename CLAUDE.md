...
- **BlendMode** (`engine_render::material`): Enum with variants `Alpha`, `Additive`, `Multiply` — declaration order IS render order via `#[derive(Ord)]`. Used as middle key in `(RenderLayer, BlendMode, SortOrder)` sort for draw batching.
...
- **Renderer trait** now includes `set_blend_mode(&mut self, mode: BlendMode)`. WgpuRenderer has no-op stub (pipeline caching deferred to GPU integration). NullRenderer/SpyRenderer implement it. SpyRenderer captures via `BlendCallLog = Arc<Mutex<Vec<BlendMode>>>` and `with_blend_capture()` builder.
