...
- **play_sound_system** (`engine_audio::play_sound_system`): `fn(ResMut<PlaySoundBuffer>, Option<Res<SoundLibrary>>, ResMut<AudioRes>)` — drains buffer, looks up each name in SoundLibrary, synthesizes SoundData at 44100 Hz / 0.5s duration, calls `audio.play()`. Skips unknown names silently. No-op when SoundLibrary absent. Registered in `Phase::PreUpdate` by DefaultPlugins (audio feature).
