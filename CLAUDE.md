...
- **SpyRenderer** (`engine_render::testing`): Test helper recording method calls via `Arc<Mutex<Vec<String>>>`. Builder pattern: `SpyRenderer::new(log)` then chain `.with_color_capture(arc)`, `.with_sprite_capture(arc)`, `.with_shape_capture(arc)`, `.with_viewport(w, h)`. Internal `log_call()` helper deduplicates lock+push. Available behind the `testing` feature flag. Shared across crates (engine_app, demo) via dev-dependency with `features = ["testing"]`.
