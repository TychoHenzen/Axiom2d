...
- **NullAudioBackend** (`engine_audio::audio_backend`): Struct with auto-incrementing `next_id: u32`. No-op impl — `play()` increments and returns `PlaybackId`, `stop()`/`set_volume()` are no-ops. Used for testing and headless mode.
- **CpalBackend** (`engine_audio::cpal_backend`): Real audio output via cpal. `Arc<Mutex<SharedState>>` holds volume, next_id counter, and active `HashSet<PlaybackId>`. `StreamHandle` wrapper makes `cpal::Stream` Send+Sync. Lazy device initialization — tests verify internal state without audio hardware. `play()` auto-increments ID and adds to active set. `stop()` removes from active set (no-op if unknown ID). `set_volume()` stores raw value.
