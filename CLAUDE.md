...
- **ActionMap** (`engine_input::action_map`): `#[derive(Resource, Debug, Clone, Default)]` mapping `String → Vec<KeyCode>`. Methods: `bind(&str, Vec<KeyCode>)` to register bindings, `bindings_for(&str) -> &[KeyCode]` to query (zero-allocation lookup via `HashMap::get(&str)`). Unbound actions return empty slice. Systems take both `Res<InputState>` and `Res<ActionMap>` as parameters.
