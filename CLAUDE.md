...
- **BlendMode** (`engine_render::material`): Enum with variants `Alpha`, `Additive`, `Multiply` — declaration order IS render order via `#[derive(Ord)]`. Used as middle key in `(RenderLayer, BlendMode, SortOrder)` sort for draw batching. `BlendMode::ALL: [Self; 3]` const array. `BlendMode::index(self) -> usize` for pipeline array indexing.
- **Material2d** (`engine_render::material`): `#[derive(Component)]` with `blend_mode: BlendMode`, `shader: ShaderHandle`, `textures: Vec<TextureBinding>`, `uniforms: Vec<u8>`. Default: Alpha blend, ShaderHandle(0), empty textures/uniforms. Attach to entities alongside Sprite or Shape; entities without Material2d default to Alpha blending.
- **ShaderHandle** (`engine_render::material`): `#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]` newtype wrapping `u32`. Returned by `ShaderRegistry::register()`.
- **ShaderRegistry** (`engine_render::material`): Plain struct with `HashMap<ShaderHandle, String>` and auto-incrementing counter. `register(&mut self, source: &str) -> ShaderHandle`, `lookup(&self, handle: ShaderHandle) -> Option<&str>`.
- **TextureBinding** (`engine_render::material`): Plain struct with `texture: TextureId` and `binding: u32`. Referenced by `Material2d::textures`.
- **effective_blend_mode** (`engine_render::material`): `pub fn(Option<&Material2d>) -> BlendMode` — returns `Material2d.blend_mode` or `BlendMode::Alpha` if no material present. Used by sprite_render_system and shape_render_system to avoid duplicating default logic.
...
- **Renderer trait** now includes `set_blend_mode(&mut self, mode: BlendMode)` and `upload_atlas(&mut self, atlas: &TextureAtlas)`. WgpuRenderer stores 3 quad pipelines + 3 shape pipelines (one per BlendMode); `set_blend_mode()` records current mode, `draw_scene_to()` partitions draws by blend mode via `compute_batch_ranges()` and switches pipelines. `upload_atlas()` replaces the default 1×1 white texture bind group with atlas data. NullRenderer no-ops both. SpyRenderer logs both; captures blend modes via `BlendCallLog = Arc<Mutex<Vec<BlendMode>>>` and `with_blend_capture()` builder.
- **sprite_render_system** now queries `Option<&Material2d>`, sorts by `(RenderLayer, BlendMode, SortOrder)`, and calls `renderer.set_blend_mode()` with deduplication (skips redundant calls when consecutive sprites share the same blend mode).
- **shape_render_system** has the same Material2d integration as sprite_render_system.
