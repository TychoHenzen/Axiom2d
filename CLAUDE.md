...
- **Flat workspace of crates**: Layout under `crates/` — `engine_core`, `engine_render`, `engine_app`, `engine_ecs`, `engine_input`, `engine_scene`, and `axiom2d` (facade + DefaultPlugins) are implemented; `engine_audio`, `engine_physics`, `engine_assets` are implemented; `engine_ui` is a placeholder. Virtual manifest at root. `demo` binary crate for smoke testing.
