...
fn when_synthesize_called_then_sound_data_has_correct_sample_rate() {
    ...
    // Assert
    assert_eq!(sound.sample_rate, 44_100, "sample rate should match the requested 44100 Hz");
}
...
fn when_synthesize_called_then_sound_data_has_mono_channel_count() {
    ...
    // Assert
    assert_eq!(sound.channels, 1, "synthesized sound should have mono (1) channel");
}
...
fn when_synthesize_called_then_sample_length_equals_frame_count_times_channels() {
    ...
    // Assert
    assert_eq!(sound.samples.len(), 44_100, "sample buffer length should equal sample_rate * duration * channels");
}
...
fn when_nonzero_amplitude_graph_then_samples_are_not_all_zero() {
    ...
    // Assert
    assert!(sound.samples.iter().any(|&s| s != 0.0), "DSP graph should produce non-zero samples from a DC 0.5 source");
}
...
fn when_synthesize_with_half_second_duration_then_frame_count_is_half_sample_rate() {
    ...
    assert_eq!(sound.samples.len(), 22_050, "sample buffer length for 0.5s at 44100 Hz mono should be 22050");
}
...
fn when_synthesize_called_twice_then_each_call_returns_fresh_sound_data() {
    ...
    // Assert
    assert_eq!(sound_a.samples.len(), sound_b.samples.len(), "two independent synthesise calls should produce same-length buffers");
}
