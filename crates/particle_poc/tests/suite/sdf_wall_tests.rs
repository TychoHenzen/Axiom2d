...
use particle_poc::{compute_distance_field, SDF_RES};
...
fn distance_field_smoke_test() {
    ...
    assert!(above > 0.0 && above < 3.0, "just above wall should be small positive, got {above}");
    // Cell far above wall (row 25): should be large positive
    let far = grid[(25 * res + 16) as usize];
    assert!(far > 5.0, "far above wall should be large positive, got {far}");
    ...
    assert!(above_14 > below_14, "gradient should point up: above={above_14} below={below_14}");
    eprintln!("EDT values: inside={inside:.2} above={above:.2} far={far:.2}");
}

fn bowl_config() -> CaptureConfig {
    CaptureConfig { width: 256, height: 256, particle_radius: 0.01, gravity: -0.3, sub_steps: 16, num_particles: 0, ..Default::default() }
}
...
fn paint_floor_with_rims(c: &mut HeadlessCapture, floor_y: f32, rim_top: f32) {
    let br: f32 = 0.10; let s: f32 = 0.008;
    let paint_y = floor_y - br;
    let left_x: f32 = -0.55; let right_x: f32 = -0.05;
    // Floor slab.
    let mut y: f32 = paint_y - br * 0.3;
    while y <= paint_y + br * 0.3 { let mut x: f32 = left_x - 0.02; while x <= right_x + 0.02 { c.paint_sdf([x,y],br,false); x+=s; } y+=s; }
    // Left rim.
    let mut x: f32 = left_x - 0.02; while x <= left_x + 0.04 {
        let mut y2: f32 = paint_y - br * 0.3; while y2 <= rim_top { c.paint_sdf([x,y2],br*0.8,false); y2+=s; }
        x+=s;
    }
    // Right rim.
    x = right_x - 0.04; while x <= right_x + 0.02 {
        let mut y2: f32 = paint_y - br * 0.3; while y2 <= rim_top { c.paint_sdf([x,y2],br*0.8,false); y2+=s; }
        x+=s;
    }
    c.upload_sdf();
...
fn when_sdf_painted_then_gpu_readback_matches() {
    let Some(mut c) = make_sdf_capture() else { return; };
...
fn when_single_particle_dropped_on_sdf_floor_then_stops() {
    ...
    for y_i in [-0.35f32, -0.40, -0.45, -0.47, -0.48, -0.50, -0.55, -0.60, -0.70] {
...
fn when_10_particles_dropped_on_sdf_floor_then_all_stop() {
    let Some(mut c) = make_sdf_capture() else { return; };
    paint_floor_with_rims(&mut c, -0.50, -0.20);
    let n = 10u32; let mut pos = Vec::new(); let mut sp = Vec::new();
    for i in 0..n { pos.push([-0.30+0.02*(i as f32).cos(),-0.38+0.015*(i as f32).sin()]); sp.push(0); }
    c.spawn_at(&pos, &sp);
    c.step_n(120);
...
fn when_30_particles_dropped_on_wide_sdf_floor_then_none_phase_through() {
    let Some(mut c) = make_sdf_capture() else { return; };
    let br: f32 = 0.06; let s: f32 = 0.005;
    // Full-width floor so horizontally ejected particles can't fall off the edge.
    let mut y: f32 = -0.62;
    while y <= -0.58 { let mut x: f32 = -0.95; while x <= 0.95 { c.paint_sdf([x,y],br,false); x+=s; } y+=s; }
    c.upload_sdf();

    let n = 30u32; let mut pos = Vec::with_capacity(n as usize); let mut sp = Vec::with_capacity(n as usize);
    for i in 0..n { let a = (i as f32/n as f32)*std::f32::consts::TAU; pos.push([-0.30+0.04*a.cos(),-0.48+0.02*a.sin()]); sp.push(1); }
    c.spawn_at(&pos, &sp);
    c.step_n(300);
    let p = c.read_positions(n);
    ...
    let through = p.iter().filter(|pt| pt[1] < -0.55).count();
    assert_eq!(through, 0, "{through}/{n} through floor");
    let pb = c.read_positions(n); c.step_n(1); let pa = c.read_positions(n);
    let md = pa.iter().zip(pb.iter()).map(|(a,b)| ((a[0]-b[0]).powi(2)+(a[1]-b[1]).powi(2)).sqrt()).fold(0.0_f32, f32::max);
