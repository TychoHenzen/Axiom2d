...
/// @doc: Verifies that when multiple keys bound to same action are pressed, the action reports as pressed.
#[test]
fn when_multiple_keys_bound_to_same_action_pressed_then_action_pressed_returns_true() {
    // Arrange
    let mut state = InputState::default();
    let mut map = ActionMap::default();
    map.bind("up", vec![KeyCode::ArrowUp, KeyCode::KeyW]);
...
#[test]
fn when_one_of_multiple_bound_keys_released_while_another_still_held_then_action_stays_pressed() {
    // Arrange
    let mut state = InputState::default();
    let mut map = ActionMap::default();
    map.bind("up", vec![KeyCode::ArrowUp, KeyCode::KeyW]);
...
#[test]
fn when_all_bound_keys_released_then_action_pressed_returns_false() {
    // Arrange
    let mut state = InputState::default();
    let mut map = ActionMap::default();
    map.bind("up", vec![KeyCode::ArrowUp, KeyCode::KeyW]);
