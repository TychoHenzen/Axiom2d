...
fn when_button_pressed_then_pressed_returns_true() {
    ...
    assert!(
        ...
        "Left button should be reported as pressed"
    );
}
...
fn when_button_pressed_then_just_pressed_returns_true() {
    ...
    assert!(
        ...
        "Left button should be just-pressed on first press"
    );
}
...
fn when_button_pressed_then_just_released_returns_false() {
    ...
    assert!(
        ...
        "just-released should be false on initial press"
    );
}
...
fn when_button_released_after_press_then_pressed_returns_false() {
    ...
    assert!(
        ...
        "released button should not be reported as pressed"
    );
}
...
fn when_button_released_after_press_then_just_released_returns_true() {
    ...
    // Assert
    assert!(
        state.just_released(MouseButton::Right),
        "released button should be reported as just-released"
    );
}
...
fn when_frame_cleared_then_just_pressed_is_false_for_held_button() {
    ...
    assert!(
        !state.just_pressed(MouseButton::Left),
        "just-pressed should be false after frame clear"
    );
    assert!(
        state.pressed(MouseButton::Left),
...
fn when_frame_cleared_then_just_released_is_false() {
    ...
    assert!(
        !state.just_released(MouseButton::Left),
        "just-released should be cleared after frame clear"
    );
}
...
fn when_cursor_moved_then_screen_pos_is_updated() {
    ...
    assert_eq!(
        ...
        "screen_pos should reflect set_screen_pos value"
    );
}
...
fn when_scroll_accumulated_then_scroll_delta_reflects_total() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::new(0.0, 3.0),
        "scroll_delta should reflect accumulated scroll"
    );
}
...
fn when_frame_cleared_then_scroll_delta_is_preserved() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::new(2.0, 5.0),
        "scroll_delta should persist across frame clear"
    );
}
...
fn when_clear_scroll_delta_called_then_scroll_delta_is_zero() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::ZERO,
        "scroll_delta should be zero after clear_scroll_delta"
    );
}
...
fn when_multiple_scroll_events_in_one_frame_then_delta_is_sum() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::new(0.0, 2.0),
        "delta should accumulate across multiple scroll events"
    );
}
...
fn when_screen_pos_set_then_clear_frame_state_does_not_reset_it() {
    ...
    assert_eq!(
        state.screen_pos(),
        Vec2::new(50.0, 75.0),
        "screen_pos should not be reset by frame clear"
    );
}
...
fn when_world_pos_set_then_world_pos_is_readable() {
    ...
    assert_eq!(
        ...
        "world_pos should reflect set_world_pos value"
    );
}
...
fn when_action_bound_to_mouse_button_and_button_pressed_then_action_pressed_returns_true() {
    ...
    assert!(
        result,
        "action_pressed should return true when bound button is pressed"
    );
}
...
fn when_action_bound_to_mouse_button_and_button_not_pressed_then_action_pressed_returns_false() {
    ...
    assert!(
        !result,
        "action_pressed should return false when no button is pressed"
    );
}
...
fn when_action_bound_to_mouse_button_and_button_just_pressed_then_action_just_pressed_returns_true()
{
    ...
    assert!(
        result,
        "action_just_pressed should return true for just-pressed bound button"
    );
}
...
fn when_button_held_but_frame_cleared_then_action_just_pressed_returns_false() {
    ...
    assert!(
        !result,
        "action_just_pressed should return false after frame clear even while held"
    );
}
...
fn when_unbound_mouse_action_queried_then_action_pressed_returns_false() {
    ...
    assert!(
        !result,
        "action_pressed should return false for unbound action"
    );
}
...
fn when_scroll_delta_zero_then_delta_is_unchanged() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::new(1.0, 2.0),
        "zero delta should not change accumulated scroll"
    );
}
...
fn when_screen_pos_set_to_origin_then_screen_pos_is_zero() {
    ...
    assert_eq!(
        state.screen_pos(),
        Vec2::ZERO,
        "screen_pos at origin should be stored correctly"
    );
}
...
fn when_unpressed_button_released_then_just_released_is_set() {
    ...
    // Assert
    assert!(
        !state.pressed(MouseButton::Middle),
        "never-pressed button should not be reported as pressed"
    );
    assert!(
...
fn when_negative_scroll_delta_accumulated_then_delta_is_negative() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::new(-10.0, -20.0),
        "negative scroll delta should be tracked correctly"
    );
}
...
fn when_scroll_with_horizontal_and_vertical_then_delta_accumulates_both_axes() {
    ...
    assert_eq!(
        state.scroll_delta(),
        Vec2::new(5.0, -8.0),
        "delta should accumulate both axes correctly"
    );
}
...
fn when_button_pressed_cleared_then_pressed_again_then_just_pressed_is_true_again() {
    ...
    assert!(
        state.pressed(MouseButton::Left),
        "button should remain pressed"
    );
    assert!(
        state.just_pressed(MouseButton::Left),
        "just_pressed should re-trigger on repeated press in new frame"
    );
}
...
fn when_rapid_press_release_repress_then_states_update_correctly() {
    ...
    assert!(
        state.just_pressed(MouseButton::Left),
        "first press: just_pressed should be true"
    );
    assert!(
        state.pressed(MouseButton::Left),
        "first press: pressed should be true"
    );
    ...
    assert!(
        !state.pressed(MouseButton::Left),
        "release: pressed should be false"
    );
    assert!(
        state.just_released(MouseButton::Left),
        "release: just_released should be true"
    );
    ...
    assert!(
        state.just_pressed(MouseButton::Left),
        "re-press: just_pressed should be true again"
    );
    assert!(
        state.pressed(MouseButton::Left),
        "re-press: pressed should be true"
    );
}
...
fn when_negative_screen_pos_set_then_screen_pos_stored_correctly() {
    ...
    assert_eq!(
        state.screen_pos(),
        Vec2::new(-100.0, -200.0),
        "negative screen_pos should be stored correctly"
    );
}
