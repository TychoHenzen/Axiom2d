...
fn when_entity_has_child_of_then_hierarchy_system_adds_it_to_parent_children() {
    ...
    assert!(
        ...
        "parent Children should contain child spawned with ChildOf"
    );
}
...
fn when_two_children_share_same_parent_then_children_contains_both() {
    ...
    assert_eq!(
        children.0.len(),
        2,
        "parent Children should contain both children"
    );
    assert!(
        children.0.contains(&child_a),
        "parent Children should contain child_a"
    );
    assert!(
        children.0.contains(&child_b),
        "parent Children should contain child_b"
    );
}
...
fn when_two_parents_each_have_one_child_then_each_parent_children_is_independent() {
    ...
    assert_eq!(
        children_a.0,
        vec![child_x],
        "parent_a should have only child_x"
    );
    ...
    assert_eq!(
        children_b.0,
        vec![child_y],
        "parent_b should have only child_y"
    );
}
...
fn when_multiple_children_belong_to_parent_then_children_vec_is_sorted_by_entity() {
    ...
    assert_eq!(
        children.0, sorted,
        "Children vec should be sorted by entity ID"
    );
    assert_eq!(
        children.0,
        vec![child_a, child_b, child_c],
        "all three children should be in order"
    );
}
...
fn when_system_runs_twice_with_no_changes_then_children_remains_stable() {
    ...
    assert_eq!(
        children.0,
        vec![child],
        "Children should remain stable on second run"
    );
}
...
fn when_child_of_is_removed_then_parent_children_no_longer_contains_that_child() {
    ...
    assert_eq!(
        children.0,
        vec![child_b],
        "removed ChildOf should remove child from parent Children"
    );
}
...
fn when_last_child_of_is_removed_then_parent_children_component_is_removed() {
    ...
    assert!(
        ...
        "parent Children should be removed when last child_of is removed"
    );
}
...
fn when_child_entity_is_despawned_then_parent_children_no_longer_contains_that_child() {
    ...
    assert_eq!(
        children.0,
        vec![child_b],
        "despawned child should be removed from parent Children"
    );
}
...
fn when_only_child_is_despawned_then_parent_children_component_is_removed() {
    ...
    assert!(
        world.get::<Children>(parent).is_none(),
        "parent Children should be removed when only child is despawned"
    );
}
...
fn when_child_is_reparented_then_old_parent_loses_child_and_new_parent_gains_it() {
    ...
    // Assert
    assert!(
        world.get::<Children>(old_parent).is_none(),
        "old parent should lose Children after child reparented"
    );
    ...
    assert_eq!(
        new_children.0,
        vec![child],
        "new parent should gain the reparented child"
    );
}
...
fn when_hierarchy_system_reruns_without_changes_then_children_are_not_marked_changed() {
    ...
    assert_eq!(
        children.0,
        vec![child],
        "unchanged hierarchy should still have correct children"
    );
}
