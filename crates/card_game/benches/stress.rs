...
/// Bake 100 unique cards (front + back) — full text layout + tessellation pipeline.
fn bench_bake_deck_stress(c: &mut Criterion) {
    ...
    let cards: Vec<(CardSignature, CardLabel)> = (0..100)
        .map(|i| {
    ...
    group.bench_function("100_cards_front_and_back", |b| {
        b.iter(|| {
