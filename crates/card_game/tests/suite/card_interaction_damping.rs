...
fn when_card_on_table_with_zero_spin_then_base_damping_applied() {
    ...
    assert!(
        matches!(&commands[0], PhysicsCommand::SetDamping { entity: e, linear, angular }
        if *e == entity && (*linear - BASE_LINEAR_DRAG).abs() < 1e-4 && (*angular - BASE_ANGULAR_DRAG).abs() < 1e-4)
    );
}
...
fn when_card_on_table_with_high_spin_then_reduced_damping_applied() {
    ...
    let entity = world
        .spawn((Card::face_down(TextureId(0), TextureId(0)), CardZone::Table))
        .id();
    let spy = SpyPhysicsBackend::new().with_angular_velocity(entity, 20.0);
    world.insert_resource(PhysicsRes::new(Box::new(spy)));
    world.insert_resource(EventBus::<PhysicsCommand>::default());
    ...
    match &commands[0] {
        PhysicsCommand::SetDamping {
            ...
            assert!(*linear < BASE_LINEAR_DRAG, "expected reduced linear drag");
            assert!(
                *angular < BASE_ANGULAR_DRAG,
                "expected reduced angular drag"
            );
        }
        other => panic!("expected SetDamping, got {other:?}"),
    }
}
...
fn when_multiple_cards_on_table_then_set_damping_called_for_each() {
    ...
    let entities: Vec<Entity> = commands
        ...
        .filter_map(|c| match c {
            PhysicsCommand::SetDamping { entity, .. } => Some(*entity),
            _ => None,
        })
        .collect();
...
fn when_card_in_reader_zone_then_set_damping_not_called() {
    ...
    let commands: Vec<_> = world
        ...
        .drain()
        .collect();
    assert_eq!(commands.len(), 1, "only the Table card should get damping");
    assert!(
        matches!(&commands[0], PhysicsCommand::SetDamping { entity, .. } if *entity == table_entity)
    );
}
