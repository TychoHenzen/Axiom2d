...
use card_game::hand::layout::{HandSpring, hand_layout_system};

// These constants are private to hand::layout, but we can derive expected values
// from the system's behavior. The tests that use them compute expected values
// from observable outputs rather than importing private constants directly.
