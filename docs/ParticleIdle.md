...
1. **✅ Particle sim core (Phase 1 complete):** GPU compute PBD + spatial hash (Morton sort + prefix scan); 100k particles at 7.5ms avg on RTX 4060; gravity, wall/container collision, positional Coulomb friction; inter-particle reaction (Red+Blue→Green via data-driven matrix); polymer bonds (Green particles form 4-bond softbody mesh); rotating conveyor arm collision with velocity transfer. Implemented in `crates/particle_poc/`. Substep count configurable via `--substeps`. Diagnostics mode (`--diagnose`) prints per-frame stats. Benchmark (`--benchmark`) measures frame time + stability. Bond tests (`--test-bond-form`, `--test-bond-constrain`, `--test-bond-break`) validate polymer behavior.
2. **✅ Machine coupling (Phase 2 complete):** Rapier2D rigid-body pipeline for machines (1 kinematic conveyor body, 2 static sensor boxes for Grinder + Heater). Multi-machine GPU collision generalized from single-conveyor OBB: `apply` pass loops over all machines, conveyors do collision+velocity transfer, sensors deferred to reaction pass. Machine sensor transmutation: Grinder converts Red(0)→Yellow(3), Heater converts Blue(1)→Purple(4). Machine state uploaded as read-only storage buffer each frame. Phase 2 adds ~0.16ms overhead (7.66ms vs 7.50ms on master). Blocked on: machine count limit (MAX_MACHINES=8), sensor boxes are static (no physics), particle consumption not yet tracked (particles are transmuted but not removed).
3. **✅ Factory/recipe layer (Phase 3 complete):** Data-driven `Recipe` struct (input/output species + count + cycle time). GPU-side atomic counter per machine (`reaction.wgsl` binding 11: `array<atomic<u32>>`; increments on sensor transmutation). CPU readback each frame via staging buffer → map-read → `MachineCpuState` (accumulated input, cycles completed, cycle timer). Heater recipe changed Blue(1)→Yellow(3) creating the Grinder(Red→Yellow)→Heater(Yellow→Purple) production chain. Machine brightness pulses with activity (0.6 idle → 1.0 active, proportional to `consumed_this_frame`). Recipe tracking works on the honor system: particles are transmuted but not consumed — they contribute to multiple cycles. Full particle consumption/emission (remove from alive-list, spawn at output) deferred to Phase 4. Small overhead: GPU atomic + CPU readback per frame, negligible at 60 FPS.
