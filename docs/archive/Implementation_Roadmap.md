...
**Implemented crates:** engine_core (37 tests), engine_ecs (1 test), engine_render (227 tests + 7 ignored GPU tests), engine_app (39 tests), engine_input (53 tests), engine_scene (42 tests), engine_audio (63 tests), engine_physics (42 tests), engine_assets (26 tests + 1 doctest), engine_ui (69 tests), axiom2d facade (15 tests), demo (41 tests). Total: 658 tests + 7 ignored GPU tests + 1 doctest.
...
Visual regression testing: HeadlessRenderer (wgpu software fallback, Rgba8Unorm offscreen texture, pixel readback with row-alignment stripping), SSIM comparison via image-compare (0.99 threshold), golden image save/load (PNG), CI integration with mesa-vulkan-drivers + WGPU_BACKEND=vulkan.
...
### Step 8.3 — Visual Regression Testing `[DONE]`
**New deps:** image-compare

- [x] Headless wgpu rendering to texture → readback pixels
- [x] SSIM comparison against golden images (0.99 threshold)
- [x] CI integration with llvmpipe software renderer
