...
fn when_empty_commands_then_returns_empty_shapes() {
    ...
    // Act
    let shapes = path_commands_to_egui_shapes(&commands, egui::vec2(100.0, 100.0), &Color::RED);
...
fn when_simple_rect_then_produces_egui_shapes() {
    ...
    // Act
    let shapes = path_commands_to_egui_shapes(&commands, egui::vec2(200.0, 200.0), &Color::GREEN);

    // Assert — should produce at least one mesh shape for the filled rect
    assert!(!shapes.is_empty(), "simple rect should produce egui shapes");
}
