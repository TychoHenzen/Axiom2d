...
/// Generate a `repository.rs` module that caches hydrated shapes and provides
/// lookup by name. `fn_names` is the list of art module names (each corresponds
/// to a sibling `{name}.rs` file with a `pub fn {name}() -> Vec<Shape>`).
pub fn generate_repository_module(fn_names: &[&str]) -> String {
    let mut out = String::new();
    ...
    for name in fn_names {
        let _ = writeln!(out, "use super::{name};");
    }
    ...
    out.push_str(
        "\n/// Cached shape repository. Call `hydrate_all` once during startup\n\
         ...
         \x20   cache: BTreeMap<&'static str, Vec<Shape>>,\n\
         }\n\n\
    ...
    for name in fn_names {
        let _ = writeln!(
            out,
            "        self.cache.insert(\"{name}\", {name}::{name}());"
        );
    }
    ...
    out.push_str(
        "    }\n\n\
         ...
         \x20       self.cache.get(name).cloned()\n\
         \x20   }\n\n\
         ...
         \x20   }\n\n\
         \x20   /// Returns an iterator over all cached (name, shapes) pairs.\n\
         \x20   pub fn iter(&self) -> impl Iterator<Item = (&&'static str, &Vec<Shape>)> {\n\
         \x20       self.cache.iter()\n\
         \x20   }\n\
         }\n",
    );
...
mod tests {
    ...
    fn when_repository_generated_then_contains_hydrate_all_and_get() {
        ...
        // Act
        let code = generate_repository_module(&names);
